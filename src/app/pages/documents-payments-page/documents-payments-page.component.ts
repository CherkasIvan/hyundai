...
export class DocumentsPaymentsPageComponent implements OnInit, OnDestroy {
  ...
  constructor(private calculateLoanService: CalculationLoanService) {}
  ...
  ngOnInit(): void {
    ...
    this.checkCascoStatus$ = this.calculateLoanService
      .checkCascoStatus(localStorage.getItem('calculation_id') as string)
      .pipe(
        repeat({ delay: 60000 }),
        filter((data: any) => {
      ...
      .pipe(
        map((data) => {
          if (this.currentDocuments[0].document_list.length === 2) {
            ...
            this.currentDocuments[0].document_list[2].documentTypeName =
              data.links[0].name;
            this.currentDocuments[0].document_list[2].url = data.links[0].link;
            this.currentDocuments[0].document_list[3].documentTypeName =
              data.links[1].name;
  ...
  attachFilesAndStartBooking(): void {
    ...
    this.calculateLoanService
      ...
      .pipe(
        ...
        switchMap(() =>
          this.calculateLoanService.startCascoBooking(
            {},
            localStorage.getItem('calculation_id') as string
          )
        ),
        switchMap(() =>
          of(this.checkCascoStatus$.subscribe((result) => console.log(result)))
        ),
        takeUntil(this._unsubscribeAll)
      )
      .subscribe((r) =>
        this.checkCascoStatus$.subscribe((result) => console.log(result))
      );
  }
